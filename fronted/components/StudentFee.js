...
const StudentFee = ({ students,onAdd, onView, onEdit, onDelete }) => {
          ...
          {students?.map(student => (
            <tr key={student._id} className="hover:bg-gray-50 transition">
              ...
              <td className="px-4 py-3 flex justify-center gap-2 text-gray-600">
                <button onClick={() => onView(student)} className="hover:text-blue-600" title="View">
                  <Eye size={18} />
                </button>
  ...
  <>
    <button onClick={() => onEdit(student)} className="hover:text-yellow-500" title="Edit">
      <Edit size={18} />
    </button>
    {!student.hasFeeGenerated && (
                  <button onClick={() => onAdd(student)} className="hover:text-green-600" title="Add Fee">
                    <Plus size={18} />
                  </button>
                )}
    <button onClick={() => onDelete(student._id)} className="hover:text-red-500" title="Delete">
      <Trash2 size={18} />
    </button>
  </>
